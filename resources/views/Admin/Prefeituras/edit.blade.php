                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="nome" id="nome" value="{{ old('nome', $prefeitura->nome) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="cnpj" id="cnpj" value="{{ old('cnpj', $prefeitura->cnpj) }}"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#009496] focus:border-[#009496]">
                            @error('cnpj')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="endereco" id="endereco" value="{{ old('endereco', $prefeitura->endereco) }}"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#009496] focus:border-[#009496]">
                            @error('endereco')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="telefone" id="telefone" value="{{ old('telefone', $prefeitura->telefone) }}"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#009496] focus:border-[#009496]">
                            @error('telefone')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $prefeitura->email) }}"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#009496] focus:border-[#009496]">
                            @error('email')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="autoridade_competente" id="autoridade_competente" value="{{ old('autoridade_competente', $prefeitura->autoridade_competente) }}"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#009496] focus:border-[#009496]">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-xs font-medium text-left text-gray-500 uppercase">Servidor Responsável</th>
                                ...
                                @foreach($prefeitura->unidades as $unidade)
                                    <tr id="unidade-row-{{ $unidade->id }}">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <button type="button" class="text-yellow-600 hover:underline edit-unidade"
...
<div id="unidade-modal" class="fixed inset-0 z-50 hidden overflow-y-auto transition-opacity duration-300 ease-in-out">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <!-- Conteúdo do Modal -->
        <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl sm:align-middle">
            <!-- Header do Modal -->
            <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                <h3 class="text-xl font-semibold text-gray-800" id="modal-title">
                    <span class="text-[#009496]">+</span> Adicionar Unidade
                </h3>
                <button type="button" id="btn-close-modal" class="text-gray-400 transition-colors duration-200 hover:text-gray-600">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('edit-unidade')) {
                ...
                fetch(`/admin/unidades/${id}`)
                    ...
                    .then(data => {
                        modalTitle.innerHTML = '<span class="text-[#009496]">✏️</span> Editar Unidade';
        ...
        form.addEventListener('submit', function(e) {
            ...
            const url = id ? `/admin/unidades/${id}` : `/admin/prefeituras/${prefeituraId.value}/unidades`;
            const method = id ? 'PUT' : 'POST';

            // Animação de loading no botão
            const submitBtn = form.querySelector('button[type="submit"]');
            const originalText = submitBtn.innerHTML;
            submitBtn.innerHTML = '<span class="flex items-center"><div class="w-4 h-4 mr-2 border-b-2 border-white rounded-full animate-spin"></div> Salvando...</span>';
